...
- 거리와 무게를 기준으로 그룹화한 2x2 매트릭스 버블 차트를 구축해,  
  **주문량과 지연율을 함께 고려한 핵심 대응 세그먼트**를 직관적으로 식별할 수 있도록 구성했습니다.

2️⃣**노선/카테고리 리스트**
![alt text](image-2.png)

- 매트릭스에서 특정 위험 구간(예: 장거리-경량)을 클릭하면,  
  하단의 **노선별 / 카테고리별 지연율 및 주문 건수**가 실시간 필터로 연동되도록 구현했습니다.

3️⃣**지연중 고객 리스트 추출**
![alt text](image-3.png)

- 매개변수 필터로 지연 일수 범위를 입력하면,  
  현재 지연을 경험 중인 **지연 고객 리스트**를 추출할 수 있도록 구성했습니다.
...
* 데이터 정합성 검증 루틴화
  - 원본 시스템과 수치를 비교하는 검증 단계를 루틴화해 신뢰도를 높이겠습니다.

* 실험형 검증 연결
  - 우선순위 세그먼트 대상 전략이 실제 지연율과 리뷰 방어에 효과가 있는지 A/B 또는 운영 테스트로 검증하겠습니다.
```

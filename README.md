...
배송 거리가 멀어질수록 지연 확률이 계단식으로 상승하는 구조적 리스크를 확인했습니다.  

### 2) 셀러의 중요성

셀러의 발송 기한 준수 여부가 최종 배송 지연율에 미치는 영향이 큽니다. (지연 셀러 vs 정시 셀러 지연율 약 4배 차이)  

### 3) 고객 만족 임계점

배송 지연 6일차부터 고객 만족도가 회복 불가능한 1점대 구간으로 진입합니다.
...
- **기대효과:**
  - 2,000km 이상의 장거리 노선(지연율 11.5%)을 풀필먼트 거점을 통해 500km 이내 구간(지연율 5.9%) 수준으로 개선할 경우 **전체 배송 지연 건수의 약 12%가 감소됩니다.**
  - 장거리 운송 비중 감소를 통해 **배송 리드타임의 변동성을 줄이고 플랫폼의 전반적인 배송 안정성을 개선할 수 있습니다.**  
